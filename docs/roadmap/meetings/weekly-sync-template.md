# Weekly Sync Template

## Meeting Info

- Week:
- Date:
- Facilitator:
- Note taker:

## This Week Focus

- Main goal:
- Current sprint:
- Main demo or verification page:

## Completed This Week

- Item 1:
- Item 2:
- Item 3:

## In Progress

| Item | Owner | Current Status | Main Blocker | Next Step |
| --- | --- | --- | --- | --- |
| Example | Foundation | In Progress | Waiting on token naming freeze | Finish provider wiring |

## Blockers

- Blocker 1:
- Blocker 2:

## Risks Changed This Week

- New risk:
- Escalated risk:
- Resolved risk:

## API Or Architecture Changes

- Decision or proposal 1:
- Decision or proposal 2:

## Demo Walkthrough

- Page or entry:
- What to verify:
- Who verifies:

## Next Week Plan

- P0:
- P1:
- P2:

## Actions

| Action | Owner | Due |
| --- | --- | --- |
| Example | Docs | Friday |

## References

- [Current Status](../execution/current-status.md)
- [Weekly Execution Board](../execution/weekly-execution-board.md)
- [Master Backlog Board](../backlog/master-backlog-board.md)
- [Owner Backlog Board](../backlog/owner-backlog-board.md)
